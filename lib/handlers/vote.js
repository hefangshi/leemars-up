...
class VoteHandler extends MsgHandler {
  ...
  _onMsg(msg) {
    ...
    if (!this.vote && msg.header.reply_to === this.watchGroup) {
      ...
      this.guess(today).then(data => {
        ...
        let verbose = `根据群主每周${date.getDay()}的上线时间\n${prevWeekDaysStr}`;
        if (data.prevFilteredDays) {
  ...
  guess(day) {
    ...
    return this.query({
      ...
      const firstOnline = self.getUserFirstOnlineTime(self.watchUser, data);
      // 根据历史时间计算本次投票时间
      return self.guessOnlineTime(day, firstOnline);
    });
  }
  ...
  guessOnlineTime(guessDay, onlineTime) {
